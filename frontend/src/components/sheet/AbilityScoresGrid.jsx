...
export function AbilityScoresGrid({
  ...
  if (isDashboard) {
    const scoreBadge =
      "mt-1 grid h-7 w-7 shrink-0 place-items-center rounded-full border border-neon-cyan/45 bg-zinc-950 p-0 text-center text-[10px] font-black leading-none tabular-nums text-starlight xl:mt-1.5 xl:h-9 xl:w-9 xl:text-sm 2xl:h-11 2xl:w-11 2xl:text-base";
    const scoreInput =
    ...
    const shell =
      "flex w-full flex-col items-center justify-center rounded-sm border border-neon-cyan/35 bg-void-panel/80 px-1 py-2 xl:min-h-[5.5rem] xl:px-2 xl:py-3 2xl:min-h-[6.75rem] 2xl:py-4";

    return (
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-6 sm:gap-2 xl:gap-3">
        {ABILITIES.map((key) => {
          ...
          if (editable) {
            return (
              <label key={key} className={`${shell} focus-within:border-neon-cyan`}>
                <span className="text-[9px] font-black uppercase tracking-wide text-zinc-500 xl:text-[11px] 2xl:text-xs">
                  {ABILITY_LABELS[key]}
                </span>
                <span className="my-1 text-xl font-black leading-none text-starlight xl:my-1.5 xl:text-3xl 2xl:text-4xl">
                  {formatModifier(mod)}
                </span>
                <input
                  type="number"
