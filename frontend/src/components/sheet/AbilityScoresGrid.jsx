...
export function AbilityScoresGrid({
  ...
  if (isDashboard) {
    const scoreBadge =
      "mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full border border-neon-cyan/45 bg-zinc-950 p-0 text-center text-[9px] font-black leading-none tabular-nums text-starlight xl:mt-1 xl:h-8 xl:w-8 xl:text-[11px] 2xl:h-9 2xl:w-9 2xl:text-xs";
    const scoreInput =
    ...
    const shell =
      "flex w-full flex-col items-center justify-center rounded-sm border border-neon-cyan/35 bg-void-panel/80 px-1 py-1.5 xl:min-h-[4.75rem] xl:px-1.5 xl:py-2 2xl:min-h-[5.25rem]";
          ...
          if (editable) {
            return (
              <label key={key} className={`${shell} focus-within:border-neon-cyan`}>
                <span className="text-[8px] font-black uppercase tracking-wide text-zinc-500 xl:text-[10px]">
                  {ABILITY_LABELS[key]}
                </span>
                <span className="my-0.5 text-lg font-black leading-none text-starlight xl:my-1 xl:text-2xl 2xl:text-3xl">
                  {formatModifier(mod)}
                </span>
                <input
                  type="number"
